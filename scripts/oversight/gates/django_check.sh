...
if "$PY" manage.py check 2>&1; then
    ...
    echo "  Common causes:"
    echo "    - Missing required environment variable: if the error above says"
    echo "      'ImproperlyConfigured: Set the SECRET_KEY environment variable'"
    echo "      (or PII_ENCRYPTION_KEY / DATABASE_URL), create a .env file from"
    echo "      .env.example — this gate runs against base settings, not test settings"
