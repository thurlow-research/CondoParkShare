...
def portal_home(request):
    ...
    recent_audit = AdminAuditLog.objects.filter(
        organization=org,
    ).select_related('actor', 'on_behalf_of').order_by('-created_at')[:20]
...
def resident_list(request):
    ...
    _log(request, 'pii_access', target_type='user', target_id=request.user.pk, notes='bulk_list')

    residents = User.objects.filter(
        organization=org,
    ).order_by('status', 'display_name')

    return render(request, 'portal/resident_list.html', {
        'residents': residents,
    })
...
def spot_list(request):
    ...
    spots = ParkingSpot.objects.filter(
        organization=org,
    ).select_related('owner').order_by('status', 'spot_number')
...
def invite_list(request):
    ...
    org = request.organization
    invites = Invite.objects.filter(organization=org).select_related(
        'issued_by', 'consumed_by'
    ).order_by('-created_at')
...
def portal_bookings(request):
    ...
    bookings = Booking.objects.filter(
        organization=org,
    ).select_related('spot', 'borrower', 'spot__owner').order_by('-created_at')
