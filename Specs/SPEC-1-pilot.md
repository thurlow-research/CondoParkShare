...
- **At-rest encryption — PII must be encrypted at rest.** *Current gap: `opus` has no disk encryption; the live DB + `audit_logs` volume are plaintext on the VHDX (being closed via CPS#104).* **Owned hardware** (opus/faberix VMs on `nexus`): **host-level BitLocker on `nexus`** satisfies the requirement — it encrypts the VHDX at rest, transparent to the VM, no auto-boot complication. **Off owned hardware** (any future VPS/cloud): **guest-level encryption under our own key is required** (e.g. LUKS on `/var/lib/docker` with TPM2 auto-unlock), since the provider controls the host layer and host encryption is not ours to trust. **Every copy that leaves the host must be encrypted** (backups via `age`; any VHDX-level image of `nexus` must be encrypted too). Re-evaluate (host→guest) on scale-up, a widening admin boundary, or a move off owned hardware. **PPE (`faberix`) mirrors the prod encryption config for parity but holds no real customer PII.** (See ADR-002; CPS#48/#63/#104.)
