...
| **`scripts/framework/install.sh`** | Installs or updates the agent pipeline framework in a target repo. Interactive: creates dirs, copies agent files, collects project-specific config values, writes `config.sh`, runs static check. Idempotent — re-running preserves existing config. | ✅ |
| **`scripts/framework/check_agents_static.sh`** | Fast structural consistency checker for the agent pipeline (no AI). Checks: agent files exist for every name referenced in docs, file paths in agent prompts are valid, escalation targets resolve, project-start output doc paths are consistent. Exit 0 = clean. Suitable for pre-commit and CI. | ✅ |
| **`scripts/framework/validate_agents.sh`** | AI-powered semantic review of agent definitions and docs. agy lens: consistency and completeness (loops, dead ends, cross-file mismatches). codex lens: adversarial gap-finding (scope-creep vectors, human-gate bypasses, missing exit conditions). Project-agnostic — reads context from `config.sh`. | ✅ |
| **`scripts/framework/run_framework_validation.sh`** | Top-level runner: static check → AI review in sequence. The single command to run before committing any framework change. | ✅ |
| **`scripts/framework/run_post_change_sweep.sh`** | Categorizes changed files by domain (framework / application-code / templates / infrastructure / design-pack / spec) and prints the agent routing plan. Entry point before invoking the `post-change-sweep` agent. | ✅ |
| **`scripts/framework/validate_docs.sh`** | AI-powered documentation coverage validator (Phase 3). Checks that docs accurately and completely describe agent behavior — catches the "omission" class: agent file says X and Y, doc only mentions X. Reads `doc-patterns.md` (known bug patterns) and `decisions.md` (verification criteria) to give reviewers explicit pattern guidance. | ✅ |
| **`scripts/framework/validate_spec_compliance.sh`** | Governance requirements compliance checker (Phase 4). Verifies that the pipeline implementation satisfies the requirements in METHODOLOGY.md and AGENTS.md: cross-vendor independence, risk-tiered thresholds, human gates, model tier assignments, loop exit conditions, mandatory self-flagging behaviors. Also checks each decision in `decisions.md` against its verification criterion. | ✅ |
| **`scripts/framework/decisions.md`** | Durable record of architectural and design decisions made during development sessions. Each entry includes the decision, rationale, implementation files, and a verification criterion that `validate_spec_compliance.sh` checks. This is the practical replacement for chat history: decisions not recorded here are invisible to future validation runs. | ✅ |
| **`scripts/framework/doc-patterns.md`** | Known documentation omission patterns discovered during sessions. Read by `validate_docs.sh` to give AI reviewers explicit pattern guidance, so previously-discovered bug classes are actively checked for recurrence rather than rediscovered. | ✅ |
| **`config.sh`** (generated) | Project-specific values for the framework scripts: `PROJECT_NAME`, `PROJECT_STACK`, `PROJECT_NON_AGENT_TOKENS`, `DESIGN_PACK_PATH`. Generated by `install.sh`. Sourced automatically by all framework scripts. Never edit manually. | ✅ |
