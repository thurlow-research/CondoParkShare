...
def test_spot_number_is_string():
    ...
    assert spot.spot_number == 'P3076', (
        f"Expected spot_number='P3076', got {spot.spot_number!r}"
    )
    assert isinstance(spot.spot_number, str), (
...
def test_spot_owner_nullable():
    ...
    assert spot.owner is None, (
        f"owner should be None, got {spot.owner!r}"
    )
...
def test_window_time_range_is_range_field():
    ...
    assert window.time_range.lower == lower, (
        f"time_range.lower mismatch: {window.time_range.lower!r} != {lower!r}"
    )
    assert window.time_range.upper == upper, (
...
def test_window_gist_index_exists():
    ...
    gist_indexes = [
        idx for idx in AvailabilityWindow._meta.indexes
        if isinstance(idx, GistIndex)
    ]
    ...
    assert 'time_range' in gist_fields, (
...
def test_booking_exclusion_constraint_in_migration():
    ...
    assert 'booking_no_overlap' in content, (
...
def test_cancelled_booking_does_not_block_rebooking():
    ...
    cancelled = BookingFactory(organization=org, spot=spot, time_range=tr, status='confirmed')
    cancelled.status = 'cancelled_owner'
    cancelled.save(update_fields=['status'])

    # New booking in same slot must succeed
    new_booking = BookingFactory(organization=org, spot=spot, time_range=tr, status='confirmed')
...
def test_booking_borrower_nullable():
    ...
    assert booking.borrower is None, (
...
def test_booking_is_anonymized_default_false():
    ...
    assert booking.is_anonymized is False, (
