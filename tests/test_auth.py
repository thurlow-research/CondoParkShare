...
class InviteFactory(factory.django.DjangoModelFactory):
    ...
    organization = factory.SubFactory(OrganizationFactory)
    issued_by = factory.SubFactory(UserFactory, organization=factory.SelfAttribute('..organization'))
...
def test_totp_enrollment_complete():
    ...
    for i, code in enumerate(plaintext_codes):
        assert check_password(code, user.recovery_codes[i]), (
    ...
    for plaintext, hashed in zip(plaintext_codes, user.recovery_codes):
        assert plaintext != hashed, "Recovery codes must be stored hashed, not plaintext"
...
def test_totp_verify_invalid_code():
    ...
    # Use '000000' as a clearly wrong token (extremely unlikely to be correct)
    wrong_token = '000000'
...
def test_totp_verify_expired_code():
    ...
    assert not device.verify_token(old_token), (
...
def test_recovery_code_cannot_be_reused():
    ...
    assert matched_again is None, "Already-consumed code should not match on second attempt"
...
def test_recovery_code_all_exhausted():
    ...
    matched = any(
        check_password(fake_code, hashed)
        for hashed in user.recovery_codes
    )
...
def test_lost_authenticator_enumerate_safe(rf):
    ...
    with patch('accounts.views.send_mail'):
        ...
        response_unknown = lost_authenticator(_make_lost_auth_request('nobody@nowhere.invalid'))

    assert response_known.status_code == 302, "Known email should redirect"
    assert response_unknown.status_code == 302, "Unknown email should redirect (enumeration-safe)"
    assert response_known['Location'] == response_unknown['Location'], (
...
def test_lost_authenticator_verify_consumed_on_use(rf):
    ...
    assert response2.status_code in (200, 302), "Second attempt response should be 200 (form error)"
...
def test_invite_registration_valid(rf):
    ...
    assert new_user.status == 'pending_totp', (
...
def test_self_registration_creates_pending(rf):
    ...
    assert new_user.status == 'pending_approval', (
...
def test_self_registration_blocked_invite_only(rf):
    ...
    from accounts.models import User
    assert not User.objects.filter(email='blocked@example.com', organization=org).exists(), (
        "No user should be created for a blocked self-registration"
    )
...
def test_default_notification_prefs():
    ...
    user = UserFactory()
    user.refresh_from_db()
    assert user.notification_prefs == {'push': False}, (
...
def test_marketing_opt_in_default_false():
    ...
    user = UserFactory()
    user.refresh_from_db()
    assert user.marketing_email_opted_in is False, (
