...
def test_erasure_scrubs_email():
    ...
    assert (
        "erased-" in user.email
    ), f"email should contain 'erased-', got {user.email!r}"
    assert (
        "@redacted.invalid" in user.email
...
def test_erasure_deletes_push_subscriptions():
    ...
    assert (
        WebPushSubscription.objects.filter(user=user).count() == 0
...
def test_erasure_scrubs_relay_bodies():
    ...
    assert RelayMessage.objects.filter(
        pk=msg_pk
    ).exists(), "RelayMessage record should still exist after erasure (audit trail)"
...
def test_erasure_preserves_booking_records():
    ...
    assert Booking.objects.filter(
        pk=booking1_pk
    ).exists(), "Booking records should still exist after erasure (only anonymised, not deleted)"
