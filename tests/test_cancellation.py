...
def test_early_release_shortens_time_range():
    ...
    release_to = _utc(
        2029, 5, 1, 13
    )  # on the hour, in the future (frozen=10:00), before end(18:00)
...
def test_early_release_frees_gate2_slot():
    ...
    from parking.booking import assign_spot, release_booking
...
def test_early_release_form_non_hour_rejected():
    ...
    with freeze_time(frozen_now):
        ...
        assert (
            not form.is_valid()
        ), "Form should be invalid when release_to is not on the hour"
        assert (
            "release_to" in form.errors
...
def test_early_release_form_past_rejected():
    ...
    with freeze_time(frozen_now):
        ...
        assert (
            not form.is_valid()
        ), "Form should be invalid when release_to is in the past"
        assert (
            "release_to" in form.errors
        ), f"Expected 'release_to' in form errors; errors: {form.errors}"
