...
def test_gate1_baseline_horizon():
    ...
    assert hours == 72, f"New user should get baseline 3*24=72h horizon, got {hours}"
...
def test_gate1_clamped_to_max():
    ...
    assert hours == 720, f"Horizon should be clamped to max 720h, got {hours}"
...
def test_gate2_completed_allows_new():
    ...
    assert result != "already_active", "Completed booking must not block a new booking"
...
def test_gate2_cancelled_allows_new():
    ...
    assert result != "already_active", "Cancelled booking must not block a new booking"
...
def test_gate3_buffer_before_blocks():
    ...
    with freeze_time(frozen_now):
        ...
        result = assign_spot(
            org, borrower, _utc(2027, 8, 10, 14, 30), _utc(2027, 8, 10, 17)
        )
...
def test_gate3_buffer_after_blocks():
    ...
    with freeze_time(frozen_now):
        ...
        result = assign_spot(
            org, borrower, _utc(2027, 8, 10, 13), _utc(2027, 8, 10, 15, 30)
        )
...
def test_gate3_outside_buffer_succeeds():
    ...
    with freeze_time(frozen_now):
        ...
        result = assign_spot(
            org, borrower, _utc(2027, 8, 10, 13, 1), _utc(2027, 8, 10, 15)
        )
...
def test_assign_spot_rotation():
    ...
    with freeze_time(frozen_now):
        ...
        owner_a = UserFactory(
            organization=org, last_booking_at=frozen_now
        )  # booked today
        ...
        AvailabilityWindowFactory(
            organization=org, spot=spot_a, time_range=window_range
        )
        AvailabilityWindowFactory(
            organization=org, spot=spot_b, time_range=window_range
        )
...
def test_assign_spot_returns_none_no_availability():
    ...
    with freeze_time(frozen_now):
        ...
        result = assign_spot(
            org, borrower, _utc(2027, 9, 10, 10), _utc(2027, 9, 10, 14)
        )
...
def test_assign_spot_tentative_status():
    ...
    assert (
        result.tentative_expires_at is not None
