...
def test_notify_skips_null_borrower():
    ...
    owner_emails = [m for m in mail.outbox if owner.email in m.to]
    assert len(owner_emails) >= 1, "Owner should still receive email when borrower is None"
...
def test_notify_skips_null_owner():
    ...
    borrower_emails = [m for m in mail.outbox if borrower.email in m.to]
    assert len(borrower_emails) >= 1, "Borrower should still receive email when owner is None"
...
def test_email_contains_local_time():
    ...
    for m in mail.outbox:
        ...
        has_local_indicator = '11:' in m.body or 'PDT' in m.body or 'PST' in m.body or 'PT' in m.body
        assert has_local_indicator, (
...
def test_owner_cancel_email_includes_reason():
    ...
    for m in borrower_emails:
        assert cancel_reason in m.body, (
...
def test_relay_send_creates_relay_message():
    ...
    assert RelayMessage.objects.filter(pk=msg.pk).exists(), (
        "RelayMessage record should exist in the database"
    )
...
def test_relay_rate_limit():
    ...
    from notifications.relay import send_relay_message, MAX_MESSAGES_PER_USER_PER_BOOKING
...
def test_relay_email_hides_real_addresses():
    ...
    assert relay_email.from_email == expected_from, (
    ...
    assert owner.email not in relay_email.body, (
        f"Owner's real email {owner.email!r} must not appear in relay email body"
    )
    assert borrower.email not in relay_email.body, (
...
def test_relay_expired_token_returns_404():
    ...
    token = uuid.uuid4()
    relay_msg = RelayMessage.objects.create(
        organization=org,
        from_user=borrower,
        to_user=owner,
        booking=booking,
